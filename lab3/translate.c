...
Exp_info translate_Exp(node_t *node, Operand *place, int direction)
{
        ...
        */
        ...
        else if (node->child_nodes[0]->type == ID_T)
        {
            ...
            if (id_node->type == ARRAY || id_node->type == STRUCTUR_USE)
            {
                ...
                InterCodes *code1 = NULL;
                if (id_node->is_param == IS_PARAM)
                    code1 = new_intercodes(place, op, NULL, ASSIGN_IC, NULL);
                else
                    code1 = new_intercodes(place, op, NULL, RIGHT_ADDR_IC, NULL);
                ret.code = code1;
                return ret;
            }
            if (direction == -1)
    ...
    else if (node->child_nodes[0]->type == ID_T)
    {
        ...
        */
        ...
        {
            tnode_t *func_node = hash_search(&symbol_hashtable, node->child_nodes[0]->text, FUNC);
            assert(func_node != NULL);
            FieldList_t *field = func_node->func->para;
            assert(field != NULL);

            Operands dummy;
            InterCodes *code1 = translate_Args(node->child_nodes[2], &dummy, field);
...
InterCodes *translate_Args(node_t *node, Operands *arg_list, FieldList_t *field)
{
    ...
    */
    ...
    arg_list->next = arg;
    int direction = 1;
    if (field->type->type == ARRAY || field->type->type == STRUCTUR_USE)
        direction = 2;
    InterCodes *code1 = translate_Exp(node->child_nodes[0], t1, direction).code;
    if (node->child_nodes[1] != NULL)
    {
        InterCodes *code2 = translate_Args(node->child_nodes[2], arg_list, field->tail);
        append_code(code1, code2);
    }
    return code1;
}
